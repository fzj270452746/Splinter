...
class MemorandumViewController: VantageViewController {
    ...
    var archives: [ContestMemorandum] = []
    ...
    func configureUI() {
        ...
        tableView.register(MemorandumTableCell.self, forCellReuseIdentifier: "ArchiveCell")
    ...
    func retrieveArchives() {
        archives = MemorandumVault.shared.retrieveAllArchives()
    ...
    @objc func obliterateAllArchives() {
        ...
        alert.addAction(UIAlertAction(title: "Delete", style: .destructive) { [weak self] _ in
            MemorandumVault.shared.eradicateAllArchives()
    ...
    func applyPadLayout(safeTop: CGFloat, safeBottom: CGFloat) {
        ...
        tableView.frame = CGRect(x: horizontalInset, y: safeTop + 90, width: maxWidth, height: view.bounds.height - safeTop - safeBottom - 280)
        emptyStateLabel.frame = tableView.frame
        obliterateAllButton.frame = CGRect(x: horizontalInset, y: view.bounds.height - safeBottom - 160, width: maxWidth, height: 55)
        withdrawButton.frame = CGRect(x: horizontalInset, y: view.bounds.height - safeBottom - 90, width: 150, height: 55)
    }
    
    func applyPhoneLayout(safeTop: CGFloat, safeBottom: CGFloat) {
        titleLabel.frame = CGRect(x: 20, y: safeTop + 10, width: view.bounds.width - 40, height: 50)
        tableView.frame = CGRect(x: 20, y: safeTop + 80, width: view.bounds.width - 40, height: view.bounds.height - safeTop - safeBottom - 250)
        emptyStateLabel.frame = tableView.frame
        obliterateAllButton.frame = CGRect(x: 20, y: view.bounds.height - safeBottom - 150, width: view.bounds.width - 40, height: 50)
        withdrawButton.frame = CGRect(x: 20, y: view.bounds.height - safeBottom - 85, width: 120, height: 50)
    }
}

extension MemorandumViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ArchiveCell", for: indexPath) as! MemorandumTableCell
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            MemorandumVault.shared.obliterateArchive(identifier: archives[indexPath.row].identifier)
            retrieveArchives()
        }
    }
}

class MemorandumTableCell: UITableViewCell {
    ...
    func configureWithArchive(_ archive: ContestMemorandum) {
