...
class GameArchiveViewController: UIViewController {
    ...
    func configureUI() {
        ...
        titleLabel.frame = CGRect(x: 20, y: 60, width: view.bounds.width - 40, height: 50)
...
extension GameArchiveViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            GameArchiveRepository.shared.obliterateArchive(identifier: archive.identifier)
            retrieveArchives()
        }
    }
}
...
class ArchiveTableCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        dateLabel.frame = CGRect(
            x: containerView.bounds.width - 150,
            y: (containerView.bounds.height - 20) / 2,
            width: 130,
            height: 20
        )
